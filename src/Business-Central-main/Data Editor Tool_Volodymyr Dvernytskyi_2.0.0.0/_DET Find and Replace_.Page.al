// page 50312 "DET Find and Replace"
// {
//     ApplicationArea = All;
...
//         if GlobalTableView <> '' then RecRef.SetView(GlobalTableView);
...
//                 if (FieldRefVar.Type() in [FieldType::Text, FieldType::Code]) and (FieldRefVar.Length() >= StrLen(FindWhat)) then begin
//                     if FieldRefVar.Class() = FieldClass::FlowField then FieldRefVar.CalcField();
//                     FieldRefVar.SETFILTER(FindWhatFilter);
//                     if not RecRef.IsEmpty() then FoundFieldList.Add(FieldRefVar.Number());
...
//                             if not GlobalWithoutValidate then FieldRefToModify.Validate();
//                             ReplacedCounter += 1;
//                         end;
//                         Rec.Insert();
//                     end;
//                     FieldRefVar.SetRange();
//                 end;
//                 if ReplacedCounter <> 0 then RecRef.Modify(not GlobalWithoutValidate);
...
//     begin
//         if GlobalTableNo = 0 then exit;
//         CustomFilterPageBuilder.AddTable(GlobalTableName, GlobalTableNo);
//         if GlobalTableView <> '' then CustomFilterPageBuilder.SetView(GlobalTableName, GlobalTableView);
...
//         else if not DataEditorMgt.GetNewColumnValue(RecRef, FieldRefVar, Rec."Record Id", TempNameValueBuffer) then exit;
//         if not GlobalWithoutValidate then FieldRefVar.Validate();
