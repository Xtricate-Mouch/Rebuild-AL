...
//         if SelectFields.RunModal() <> Action::LookupOK then exit;
...
//         if not DataEditorMgt.GetNewColumnValue(RecRef, NewFieldRef, Rec."Source Record ID", TempNameValueBuffer) then exit;
//         if not Confirm(ColumnUpdateConfirmLbl, false, TempDETField.Name, RecRef.Count(), RecRef.Name()) then exit;
//         if RecRef.FindSet() then
//             repeat
//                 FieldRefVar := RecRef.Field(TempDETField."Field Id");
//                 FieldRefVar.Value(NewFieldRef.Value());
//                 if not WithoutValidate then FieldRefVar.Validate();
...
//         if not (PageKey.RunModal() in [Action::LookupOK, Action::OK]) then exit;
...
//         if not (InsertNewRecordPage.RunModal() in [Action::LookupOK, Action::OK]) then exit;
...
//     begin
//         if SourceRecRef.Get(SourceRecordID) then SourceRecRef.Delete(not WithoutValidate);
//     end;
...
//             SkipField := (FieldRefVar.Class() = FieldClass::FlowFilter) or ((FieldRefVar.Class() = FieldClass::FlowField) and ExcludeFlowFields) or (FieldRec.IsEmpty);
...
//         if CustomTableView <> '' then RecRef.SetView(CustomTableView);
//         if RecRef.FindSet() then
//             repeat
//                 FieldCount := 0;
//                 Counter := 0;
...
//                         if FieldRefVar.Class() = FieldClass::FlowField then FieldRefVar.CalcField();
...
//     begin
//         if not RecRef.Get(Rec."Source Record ID") then exit;
//         GenFieldInfoDict.Get(FieldCounter, FieldInfo);
//         foreach OriginalFieldNo in FieldInfo.Keys() do FieldRefVar := RecRef.Field(OriginalFieldNo);
...
//         if not DataEditorMgt.GetNewColumnValue(RecRef, FieldRefVar, Rec."Source Record ID", TempNameValueBuffer) then exit;
//         if not WithoutValidate then FieldRefVar.Validate();
//         RecRef.Modify(not WithoutValidate);
...
//     begin
//         if FieldCounter > GenFieldInfoDict.Count() then exit(false);
//         GenFieldInfoDict.Get(FieldCounter, FieldInfo);
//         foreach FieldTypeAsText in FieldInfo.Values() do exit(FieldTypeAsText in [Format(FieldType::Text), Format(FieldType::Code), Format(FieldType::Integer), Format(FieldType::Decimal), Format(FieldType::Boolean), Format(FieldType::Date), Format(FieldType::DateFormula), Format(FieldType::DateTime), Format(FieldType::Time), Format(FieldType::Guid), Format(FieldType::BigInteger)]);
//     end;
