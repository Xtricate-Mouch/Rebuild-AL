codeunit 50308 "Approvals Codeunit"
{
    ...
    begin
        case RecRef.Number() of
            Database::"Inspection Receipt Header B2B":
                begin
                    RecRef.SetTable(InspectionReceipt);
                    InspectionReceipt."Approval Status" := InspectionReceipt."Approval Status"::Approved; //release "old text"
                    InspectionReceipt.Modify();
                    Handled := true;
                end;
        end;
    ...
    begin
        case RecRef.Number() of
            Database::"Inspection Receipt Header B2B":
                begin
                    RecRef.SetTable(InspectionReceipt);
                    InspectionReceipt."Approval Status" := InspectionReceipt."Approval Status"::Rejected; // pending to release "old text"
                    InspectionReceipt.Modify();
                    IsHandled := true;
                end;
        end;
...
// Kumari -----------
codeunit 50309 "Purchase Qoute Codeunit"
{
...
codeunit 50310 "Routing Approvals"
{
    Subtype = Normal;

    procedure CheckRoutingHeaderApprovalsWorkflowEnabled(var RoutingHeader: Record "Routing Header"): Boolean
    begin
        // Add logic to check if workflow is enabled
        exit(true); // Temporary — replace with real check
    end;

    procedure OnSendRoutingHeaderForApproval(var RoutingHeader: Record "Routing Header")
    begin
        // Add logic to send for approval
        Message('Routing Header %1 sent for approval.', RoutingHeader."No.");
    end;

    procedure OnCancelRoutingHeaderForApproval(var RoutingHeader: Record "Routing Header")
    var
    ...
    begin
        ...
        WorkflowStepInstance.SetRange("Record ID", RoutingHeader.RecordId);

        // Cancel all approval requests for this Routing Header record
