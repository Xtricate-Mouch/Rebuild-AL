...
codeunit 50311 "Req. Wksh.-Make Order MTAR"
{
    ...
    var
        ...
        ReqWkshMakeOrders: Codeunit "Req. Wksh.-Make Order MTAR";
        ...
        PostingDateReq: Date;
        ReceiveDateReq: Date;
        "Sales Order No. Cus": Code[20];
        EndOrderDate: Date;
        PrintPurchOrders: Boolean;
        PrintPurchIndent: Boolean;
    ...
    procedure Set(NewPurchOrderHeader: Record "Purchase Header"; NewEndingOrderDate: Date; NewPrintPurchOrder: Boolean; NewPrintPurchIndent: Boolean)
    begin
        PurchOrderHeader := NewPurchOrderHeader;
        EndOrderDate := NewEndingOrderDate;
        PrintPurchOrders := NewPrintPurchOrder;
        PrintPurchIndent := NewPrintPurchIndent; //B2BKM26AUG2024
    ...
    var
        ...
        IsHandled: Boolean;
        PLanningCreatePurch: Enum "Planning Create Purchase Order";
    begin
        IsHandled := false;
    ...
    begin
        ...
        if PLanningCreatePurch = PLanningCreatePurch::"Make Purch. Orders" then//B2BKM03SEP2024
    ...
    begin
        ...
        with ReqLine do begin
            ReqWkshMakeOrders.Set(PurchOrderHeader, EndOrderDate, PrintPurchOrders, PrintPurchIndent); //B2BKM26AUG2024
    ...
    begin
        ...
        if IsHandled then
            exit;
        if PrintPurchIndent = false then begin //B2BKM26AUG2024
        ...
        End else if //B3BKM26AUG2024 <<
          PrintPurchIndent = true then begin
            with RequisitionLine do begin
                Clear(PurchOrderLine);
                PurchOrderLine.Init();
                PurchOrderLine.BlockDynamicTracking(true);
    ...
    var
        ...
        Msg_001: Label 'Purchase Order %1 is Created';
        Msg_002: Label 'Purchase Indent %1 is Created';
    begin
        OnBeforeInsertHeader(ReqLine2, PurchOrderHeader, OrderDateReq, PostingDateReq, ReceiveDateReq, ReferenceReq);
        if PrintPurchIndent = false then begin //B3BKM26AUG2024
            with ReqLine2 do begin
                OrderCounter := OrderCounter + 1;
                if not PlanningResiliency then
                    if not HideProgressWindow then
                ...
                PurchOrderHeader."Sales Order No." := "Sales Order No. Cus";
            ...
            Message(Msg_001, PurchOrderHeader."No.");
        end else if PrintPurchIndent = true then begin //B3BKM26AUG2024<<
            with ReqLine2 do begin
                OrderCounter := OrderCounter + 1;
                if not PlanningResiliency then
                    if not HideProgressWindow then
                ...
                PurchOrderHeader."Your Reference" := ReferenceReq;
                PurchOrderHeader."Order Date" := OrderDateReq;
                PurchOrderHeader."PO Indent" := true;
                PurchOrderHeader."Expected Receipt Date" := ReceiveDateReq;
                PurchOrderHeader."Sales Order No." := "Sales Order No. Cus";
                ValidateBuyFromVendorNo(PurchOrderHeader, ReqLine2);
                if "Order Address Code" <> '' then
            ...
            Message(Msg_002, PurchOrderHeader."No.");
        end; //B3BKM26AUG2024 >>>
    end;
